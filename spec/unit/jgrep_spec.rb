...
describe JGrep do
    describe "#jgrep" do
        ...
        it "should correctly create the JGrep class" do
             result = JGrep::JGrep.new
             result.value.should == ""
             result.expression.should == ""
         end

        it "should correctly set JGrep object members" do
            result = JGrep::JGrep.new
            result.value = "foo"
            result.expression = "bar"
            result.value.should == "foo"
            result.expression.should == "bar"
        end

        it "should correctly match predefined value on expression" do
            jgrep = JGrep::JGrep.new
            jgrep.value = {"foo" => "bar"}
            result = jgrep.match_expression("foo=bar")
            result.should == true
        end

        it "should correctly match predefined expression to value" do
            jgrep = JGrep::JGrep.new
            jgrep.expression = "foo=bar"
            result = jgrep.match_value({"foo" => "bar"})
        end
